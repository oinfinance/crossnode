...
unsignedTx="unsigned_map_register.json"
signedTx="signed_map_register.json"


curl -s  -X POST --data-binary "{\"base_req\":{\"from\":\"$from\",\"chain_id\":\"$CHAINID\",\"fees\":[{\"denom\":\"feecoin\",\"amount\":\"100\"}],\"generate_only\":true},\"erc_addr\":\"$test_oin_erc20_addr\",\"cc_addr\":\"${test_cross_chain_address}\"}" ${rest_laddr}/mapping/register/ > $unsignedTx
