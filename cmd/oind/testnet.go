...
func testnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Oin testnet",
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "oind","Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "oincli",	"Home directory of the node's cli configuration")
...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	oinConfig := srvconfig.DefaultConfig()
	oinConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		serverConfigFilePath := filepath.Join(nodeDir, "config/oind.toml")
		srvconfig.WriteConfigFile(serverConfigFilePath, oinConfig)
	}
