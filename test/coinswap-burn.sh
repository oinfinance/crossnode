...
curl -s  -X POST --data-binary "{\"base_req\":{\"from\":\"$from\",\"chain_id\":\"$CHAINID\",\"fees\":[{\"denom\":\"feecoin\",\"amount\":\"100\"}],\"generate_only\":true},\"txHash\":\"$test_txhash\",\"fromChain\":\"$test_fromchain\", \"fromAddr\":\"$test_fromaddr\",\"token\":\"$test_token\",\"value\":$test_value,\"toAddr\":\"$test_toaddr\",\"toChain\":\"$test_tochain\"}" ${rest_laddr}/coinswap/burn/ > $unsignedTx
